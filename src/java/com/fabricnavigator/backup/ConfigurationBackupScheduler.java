...
public final class ConfigurationBackupScheduler implements ServletContextListener {
    ...
    public static final class Record {
        public String id="",device="",platform="",softwareVersion="",capturedAt="",capturedBy="",source="",changeActor="",sha256="",archiveSha256="";
    ...
    public static int[] captureAll(String actor,String source,String remote) throws Exception{
        ...
        for(final String device:devices)completed.submit(new Callable<BackupOutcome>(){public BackupOutcome call(){try{capture(device,actor,source,remote);return new BackupOutcome(device,true,"");}catch(Exception ex){String error=safeMessage(ex);try{AuditLog.log(actor,"FAILED_CONFIG_BACKUP",device+" · "+error,remote);}catch(Exception ignored){}return new BackupOutcome(device,false,error);}}});
        try{for(int index=0;index<devices.size();index++){BackupOutcome outcome=completed.take().get();if(outcome.success)ok++;else{failed++;failures.add(outcome.device+" · "+outcome.error);}writeStatus("running","Completed "+outcome.device,ok,failed,devices.size(),failures);}}finally{workers.shutdownNow();}
        writeStatus(failed>0?"warning":"success",ok+" device(s) backed up · "+failed+" failed",ok,failed,devices.size(),failures);return new int[]{ok,failed};
    }
    ...
    public static Record capture(String device,String actor,String source,String remote) throws Exception{
        ...
        validateDevice(device);synchronized(deviceLock(device)){
            ...
            String first=result.output.substring(0,marker),platform=first.indexOf("FN_PLATFORM=switchengine")>=0?"switchengine":first.indexOf("FN_PLATFORM=fabricengine")>=0?"fabricengine":"",softwareVersion=markerValue(first,"FN_SOFTWARE_VERSION");if(platform.length()==0)throw new IOException("Unsupported device platform");
            ...
            String configuration=normalize(result.output.substring(marker+"FN_CONFIG_BEGIN\n".length()));if(configuration.length()==0)throw new IOException("The device returned an empty configuration");if(softwareVersion.length()==0)softwareVersion=configurationVersion(configuration);
            ...
            boolean unchanged=latest!=null&&digest.equals(latest.sha256)&&archiveDigest.equals(latest.archiveSha256)&&(softwareVersion.length()==0||softwareVersion.equals(latest.softwareVersion));
    ...
    public static String displayConfiguration(String device,String id) throws Exception{
        Record record=find(device,id);
        if("fabricengine".equals(record.platform)&&record.archive!=null&&Files.isRegularFile(record.archive))return archiveConfiguration(device,id);
        return new String(Files.readAllBytes(record.configuration),StandardCharsets.UTF_8);
    }
    ...
    private static final class BackupOutcome{final String device,error;final boolean success;BackupOutcome(String device,boolean success,String error){this.device=device;this.success=success;this.error=error;}}
    ...
    private static String markerValue(String value,String marker){for(String line:value.replace("\r","").split("\n")){String prefix=marker+"=";if(line.startsWith(prefix))return clean(line.substring(prefix.length()));}return "";}
    private static String configurationVersion(String value){java.util.regex.Matcher matcher=java.util.regex.Pattern.compile("(?im)^\\s*#?\\s*(?:software\\s+version|software\\s+release|image\\s+version|extremexos\\s+version|version)\\s*(?::|=|is)?\\s*[vV]?([0-9]+(?:\\.[0-9A-Za-z_-]+){1,7})\\s*$").matcher(value);return matcher.find()?clean(matcher.group(1)):"";}
