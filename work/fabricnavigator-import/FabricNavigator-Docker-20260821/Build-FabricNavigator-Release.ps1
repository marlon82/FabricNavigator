...
try {
    ...
    $overlay = [ordered]@{
        ...
        (Join-Path $PSScriptRoot 'service-openapi.py') = 'opt/fabricnavigator/service-openapi.py'
        (Join-Path $PSScriptRoot 'config-backup.pl') = 'opt/fabricnavigator/config-backup.pl'
        (Join-Path $PSScriptRoot 'firmware-deploy.pl') = 'opt/fabricnavigator/firmware-deploy.pl'
    ...
    foreach ($relative in @(
        ...
        'login.jsp', 'setup.jsp', 'setup-credentials.jsp', 'topology\index.jsp', 'topology\service-action.jsp', 'topology\ping-action.jsp', 'topology\config-backup-action.jsp', 'snmp\get-variables.jsp', 'devices\index.jsp', 'credits\index.jsp', 'config-backups\index.jsp', 'firmware\index.jsp', 'index.jsp', 'admin\index.jsp',
...
'@.Replace('__TOMCAT_VERSION__', $tomcatVersion)
    ...
    Copy-Item -LiteralPath (Join-Path $sourceRoot 'third_party\xtermjs\LICENSE-MIT.txt') -Destination (Join-Path $runtimeLicenses 'XTERMJS-LICENSE-MIT.txt')
